// A RuleResult xUnit v3 can serialize, so discovery lists one test per rule, and whose Assert
...
/// <summary>A <see cref="RuleResult"/> that xUnit v3 serializes and that fails with <see cref="FailException"/>.</summary>
public sealed class XunitRuleResult : RuleResult, IXunitSerializable
{
    ...
    public void Deserialize(IXunitSerializationInfo info)
    {
        ...
        string json = info.GetValue<string>(Key)
            ?? throw new RulebearingException("A serialized xUnit row has no rule result. Fix: rebuild the test project so discovery and execution agree.");
        Restore(FromJson(json), Failure);
    }
