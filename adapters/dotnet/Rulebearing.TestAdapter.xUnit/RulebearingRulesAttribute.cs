// xUnit v2 data source: one theory row per rule of a Rulebearing run
// (docs/plans/pending/0002-wave-2-dotnet-python-element-rules.md, Step 14;
// docs/artifacts/archunitnet-0.13.4-coverage.md#test-framework-adapters, ArchUnitNET.xUnit).
...
/// <summary>
/// Supplies one <see cref="RuleResult"/> per rule to an xUnit v2 <c>[Theory]</c>, so
...
public sealed class RulebearingRulesAttribute : DataAttribute
{
    ...
    public override IEnumerable<object[]> GetData(MethodInfo testMethod) =>
        RulebearingRun.Load(Options).Select(static rule => new object[] { new XunitRuleResult(rule) });
}
