// The fixture repository as an MSTest V2 user writes it: one data row per rule. The failing rule
...
namespace Rulebearing.TestAdapter.MSTestV2.Tests;
...
public sealed class ArchitectureRules
{
    ...
    public void Holds(RuleResult rule)
    {
        ...
        {
            AssertFailedException failure = Assert.ThrowsException<AssertFailedException>(rule.Assert);
