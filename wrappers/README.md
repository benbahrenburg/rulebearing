...
The npm wrapper is the real package: its launcher, tests and staging script are described in [npm/README.md](npm/README.md), and [release.yml](../.github/workflows/release.yml) stages, install-checks and publishes it from the release archives ([docs/release.md](../docs/release.md#the-npm-wrapper-from-wave-1)). Until wave 2 lands, the NuGet and PyPI directories hold the `0.0.1` name reservation: a README and the packaging metadata, no functionality. [`crates/rulebearing/`](crates/rulebearing/README.md) is the crates.io reservation. [`publish-placeholders.sh`](publish-placeholders.sh) published all four reservations on one day ([Wave 0, 0E](../docs/plans/pending/0000-wave-0-spike.md); the procedure is in [docs/release.md](../docs/release.md)); its npm step now packs the real wrapper, so it is not rerun for npm.
