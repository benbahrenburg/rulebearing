...
# conformance/archunitnet/ported.json's and conformance/netarchtest/ported.json's `ported` may not
# fall, and the layer 1 threshold in
...
netarchtest_now="$(python3 -c 'import json; print(json.load(open("conformance/netarchtest/ported.json"))["ported"])')"
...
netarchtest_was="$(base_value conformance/netarchtest/ported.json 'd["ported"]')"
if [ -z "$netarchtest_was" ]; then
  netarchtest_was="$netarchtest_now"
  echo "ratchets: $base has no netarchtest/ported.json recorded at this pin; this change sets the starting count ($netarchtest_now)"
fi
...
echo "ratchets: excluded.json base=$excluded_was now=$excluded_now; ported base=$ported_was now=$ported_now; netarchtest ported base=$netarchtest_was now=$netarchtest_now"
...
if [ "$netarchtest_now" -lt "$netarchtest_was" ]; then
  echo "::error::netarchtest ported fell from $netarchtest_was to $netarchtest_now; it may only rise (ADR-0009)"
  status=1
fi
exit "$status"
