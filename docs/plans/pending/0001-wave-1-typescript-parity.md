...
| 1E | Hooks, `summary --format agent`, `impact` | Done | `cmd/{hooks,summary,impact}.rs`; merging is idempotent and keeps other settings; both `--from-hook` forms answer in Claude Code's hook protocol and exit 0: `impact` as `additionalContext`, the `Stop` cruise as `{"decision": "block"}` on errors and silent under `stop_hook_active` ([docs/agents.md](../../agents.md#the-hooks), tested in [`tests/agent.rs`](../../../crates/rb-cli/tests/agent.rs)); a saved result gates under `fmt --exit-code` as the cruise did ([ADR-0031](../../adr/0031-a-saved-result-carries-what-the-exit-code-counts.md)) |
...
| 1F | `adopt`: baseline, CI step, hook, doc, green PR | Done | `cmd/adopt.rs` and [`tests/first_run.rs`](../../../crates/rb-cli/tests/first_run.rs): a three-violation repository goes green, the branch is committed, and the PR body is a snapshot. The PR on a repository with a non-empty baseline is an external action taken by the maintainer, closed on the maintainer's instruction on 2026-09-24; the PR link is not recorded here |
...
| Sub-wave | Item | Status | Evidence |
| --- | --- | --- | --- |
| 1G | Layer 5 and the twelve-mutation branch green | Done | Three oracles at zero differences with equal exit codes and the mutation branch 12 of 12 in both tools, green on `main` in the [`conformance-gate-1-layer-5` job](https://github.com/benbahrenburg/rulebearing/actions/runs/35894213624/job/107293969162) and in the [nightly](https://github.com/benbahrenburg/rulebearing/actions/runs/35894226485); [`conformance/divergences.md`](../../../conformance/divergences.md) empty |
| 1G | Zero-diff: dependency-cruiser, langfuse, FluidFramework | Done | 573, 3,832 and 656 modules; 26, 369 and 1 violations; 0 differences each. nightly rows: "yes" for all three, Rulebearing 0.20 s, 2.13 s and 0.19 s (median of three), in the [nightly of 2026-09-23](https://github.com/benbahrenburg/rulebearing/actions/runs/35897816322) on `main`, published to the `testbeds-results` branch |
| 1G | `v0.1.0`: Releases, npm, Action in use here | In progress | The workspace and the npm wrapper are at 0.1.0; the [release dry run](https://github.com/benbahrenburg/rulebearing/actions/runs/35900317703) on `main` built the six targets, packed the seven npm packages and ran `npx rulebearing --version` on macOS, Linux and Windows, publishing nothing. The `NPM_TOKEN` secret is set. Remaining: the `v0.1.0` tag, on the maintainer's confirmation, then the Action at `v0.1.0` in this repository's CI |
| 1G | `docs/perf.md`: private monorepo and synthetic figures | Done | Synthetic, on `ubuntu-latest`: mean: 0.966 s, p95: 1.005 s over 10 runs at `f2e4804` ([bench run](https://github.com/benbahrenburg/rulebearing/actions/runs/35894230077)). Private monorepo, 5,838 modules on an Apple M2 Pro: 3.07 s mean, 3.36 s p95, against dependency-cruiser's 10.64 s, with 0 differences between the two results; over 2 s, with the miss in extraction (2.0 s) recorded as a wave 2 item ([docs/perf.md](../../perf.md#the-private-monorepo)) |
| 1G | Upstream offer issue; `docs/adoption.md` baseline | Done | The baseline is in [docs/adoption.md](../../adoption.md), with `scripts/adoption-signals.sh`. The upstream offer issue is an external action taken by the maintainer, closed on the maintainer's instruction on 2026-09-24; the issue link is not recorded here |
