...
- As built, the key extends those three inputs with the tool version and a working-tree fingerprint (the entries of `git status --porcelain -z` with their bytes, or path, size and mtime outside git, plus the extractor manifests such as `package.json`, `tsconfig.json`, `pyproject.toml` and every `.csproj`), because the final review showed an uncommitted edit was otherwise answered from the committed graph. An entry that does not deserialise as a graph is a miss, and the newest eight entries per worktree are kept.
...
| 2C | `unported.json` lists only `custom-predicate`: diagram generation | Deferred | approved by the maintainer, 2026-09-24. The 12 `wave-3` entries (`PlantUmlFileBuilderTest`, the generation half of `PlantUmlFluentComponentDiagramTests`) test the `plantuml` reporter, which section 1.2 puts in wave 3; delivered with that reporter in [plan 0003](0003-wave-3-operations-surface-inner-loop.md), whose round trip (`--output-type plantuml`, then `adhereTo` the file) is where they are ported |
| 2C | `unported.json` lists only `custom-predicate`: C# API surface | Waived | approved by the maintainer, 2026-09-24. The 7 ArchUnitNET `api-only` entries test the C# fluent API itself (rendered rule descriptions and `As()`, `IArchRule` equality, the `ConjunctionFactory` reflection factory, the XML violation store); a declarative rule has no counterpart, and each entry argues it in `conformance/archunitnet/unported.json` |
| 2C | `unported.json` lists only `custom-predicate`: NetArchTest semantics | Waived | approved by the maintainer, 2026-09-24. The 78 NetArchTest entries (38 `constructed-type`, 16 `dependency-definition`, 8 `vocabulary-gap`, 16 `api-only`) are places where NetArchTest's definitions differ from ArchUnitNET's, which the element rules follow; the .NET oracle harness found no disagreement that depends on them (6 of 9 oracles compared, 0 disagreeing). Each entry argues its reason in `conformance/netarchtest/unported.json`; the `gate2-ratchet` check refuses any `not-yet` |
...
| 2D | `rulebearing:dotnet`, `rulebearing:python`, `recommended` composition, `--init` presets | Done | `cargo test -p rb-config --test presets` (seven snapshots; `recommended` spells out the union of the three presets and a test holds it to them); `cargo test -p rb-cli --test init_presets`, `--test config_expand`; `help-cruise.txt`, `help-init.txt` snapshots. `rulebearing:recommended` composing the three presets, which changes the graph for existing users of it, was kept by the maintainer on 2026-09-24 |
...
| 2E | `metrics`, `--metrics` | Done | gate 1 layer 3: `metrics.spec.mjs` (11) and the comparison with upstream's code over `hideFolders`, `hideModules`, `orderBy`, 0 differing; `--metrics` / `--no-metrics` in `graph_reporters.rs`. `fmt --exit-code -T metrics` on a result without folders exits 0 as ADR-0030 lists the gating reporters, where upstream exits 1; the maintainer kept ADR-0030's rule on 2026-09-24 |
...
| 2F | .NET oracle harness and table (nine repos) | In progress | `testbeds/oracles/dotnet.sh`, `compare.py dotnet` (tested in `testbeds/oracles/test_compare.py`), the `oracle-dotnet` nightly job and the README table checked by the `oracle-table` CI job. Local results, committed under `testbeds/results/`: no disagreement on any row; three agree (onebeyond/monaco 6, phongnguyend/Practical.CleanArchitecture 5, evolutionary-architecture-by-example 2); three compare nothing, every test staying with the incumbent or not importing (ardalis/RiverBooks, karaoke-dev/karaoke, NeVeSpl/NetArchTest.eNhancedEdition); three cannot run at the pinned SHA (DrJohnMelville/Pdf does not build, nager/Nager.Date runs no tests, dennisdoomen/packageguard's tests use neither library). Open: three consecutive green nightly runs |
| 2F | Python oracle harness and table (fourteen repos, incl. dify and OpenMetadata halves) | In progress | `testbeds/oracles/python.sh`, run locally 2026-09-24 at the pinned SHAs with the release binary ([`testbeds/results/`](../../../testbeds/results/), README table regenerated): 13 of 14 rows compared, 86 contracts, 0 disagree (84 agree, 2 `stays`: kopf's `conditional` and openedx-platform's `isolated_apps`, custom contract types). Before, 12 of the 86 disagreed on 8 repositories, each because import-linter narrows a contract's graph (`ignore_imports`, `exclude_type_checking_imports`, folders without `__init__.py`, modules outside the root packages); the importer now writes those as each rule's `graph` ([ADR-0038](../../adr/0038-a-rule-narrows-the-graph-it-sees.md)). OpenMetadata stays `error`: import-linter itself cannot run there without `pydantic`. Open: three consecutive green nightly runs |
...
| 2G | Worktree-aware cache key | Done | `crates/rb-cli/tests/cache_worktree.rs` (two worktrees at different `HEAD`s: different directories, neither reads the other's graph; a commit and `git pack-refs` handled; an uncommitted edit or an untracked file gives a new entry); `crates/rb-cli/src/cache/key.rs` unit tests |
...
| 2H | Release workflow publishes NuGet and PyPI | In progress | `release.yml` jobs `version`, `pypi-build`, `pypi-install-check`, `pypi-publish`, `nuget-pack`, `nuget-install-check`, `nuget-publish`, from one tag at one version; actionlint clean. Dry run green on 2026-09-25 ([run 36093513255](https://github.com/benbahrenburg/rulebearing/actions/runs/36093513255)): the six binaries, the npm, PyPI and NuGet packages, and the nine install checks on Linux, macOS and Windows, with the release and publish jobs skipped. Secrets set 2026-09-25 and proven without publishing by the manual run's `pypi-credentials` and `nuget-credentials` jobs ([run 36095882191](https://github.com/benbahrenburg/rulebearing/actions/runs/36095882191)): each re-uploads the 0.0.1 reservation, which the registry refuses before reading it when the token is missing or scoped away. Open: `pytest-rulebearing` and the seven `Rulebearing.TestAdapter*` packages do not exist yet, so the first release needs `PYPI_TOKEN` to cover a new project (an account-wide token, or the name reserved first) and `NUGET_API_KEY`'s glob to allow new packages ([docs/release.md](../../release.md#pypi-from-wave-2)); then the first `vX.Y.Z` tag |
