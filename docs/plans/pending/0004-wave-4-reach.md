...
Repeat step 1 at the end of the wave: the six signals for the two months of the wave, the usage-count aggregates if any repository opted in, the greenfield issues opened in wave 3 and their outcomes, and whether a maintainer accepted a proposed rule set. Record which of the three outcomes in § 1.2 applies going forward and open the ADR or plan it calls for. The same review decides whether to make the four oracle offers that [plan 0002, Step 15](0002-wave-2-dotnet-python-element-rules.md#215-step-15-greenfield-init-proof-the-nightly-tables-upstream-offers-second-maintainer-2i) deferred (evolutionary-architecture-by-example, RiverBooks, kedro, sqlfluff; deferred by the maintainer, 2026-09-24, because no outreach to other projects is made yet), and the second-maintainer invitation that was to go with them. No offer is made before the maintainer decides it here.
