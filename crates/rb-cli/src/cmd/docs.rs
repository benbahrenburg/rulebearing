...
use rb_config::capability::{Capability, capability};
use rb_config::elements::{Concept, Side, VOCABULARY, ValueKind, spellings, split_key};
...
mod tests {
    ...
    fn the_reference_has_a_row_per_concept_and_the_committed_file_is_current() {
        ...
        assert!(
            sealed.contains("| yes | no: the language cannot forbid subclassing |"),
            "{sealed}"
        );
