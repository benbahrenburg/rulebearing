...
//! current wave are delivered; asking for a later one is a named error. Wave 2E adds the graph
//! reporters ([`dot`], [`mermaid`], [`d2`]), [`metrics`] and [`err_html`].
...
pub enum ReportError {
    ...
    #[error(
        "the `{name}` reporter arrives in wave {wave}; use err, err-long, err-html, json, text, csv, teamcity, azure-devops, github-annotations, agent, dot, ddot, archi, cdot, flat, fdot, mermaid, d2, metrics or null"
    )]
    NotYet {
        /// The type.
        name: String,
